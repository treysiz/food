...
<head>
<meta charset="UTF-8">
<title>厨房食材管理系统 Kitchen Inventory</title>
...
<div class="alert success">✔ 食材保存成功 / Saved Successfully (<?= $_GET['saved'] ?> bytes)</div>
<?php endif; ?>
...
<?php foreach ($foods as $i=>$f):
    ...
    <div class="card <?= $c['status'] ?>" data-category="<?= $f['category'] ?>">
        ...
        <div class="left">剩余 / Left：<?= $c["left"] ?> 天 Day <?= $c["hours"] ?> 小时 H <?= $c["mins"] ?> 分 Min</div>

        <?php if ($f['auto_renew'] ?? false): ?>
            <div class="renew">🔄 自动续期中 / Auto-Renew ON</div>
        <?php endif; ?>
    </div>
<?php endforeach; ?>
</div>
...
<div class="login-box">
    ...
    <form method="post"><input name="login_password" type="password" placeholder="输入密码 / Enter Password"><button>登录 Login</button></form>

    <p>📱 手机扫码登录后台 / Scan QR to Login</p>
    <div id="qr-login"></div>
...
<div class="admin-box">
    ...
    <a href="?view=1" class="btn-link">切换展示模式 / View Mode</a>
    <a href="?logout=1" class="btn-logout">退出登录 / Logout</a>
    <hr>
    ...
    <form method="post">
        ...
        <input name="name_en" placeholder="英文名称 (可选) / English Name">
    ...
    <h2>📋 当前食材 / Current Items</h2>
    <?php foreach ($foods as $i=>$f): ?>
        <form method="post" style="margin-bottom:10px;">
            <b><?= $i+1 ?>. <?= htmlspecialchars($f["name"]) ?></b> （<?= $f["start_date"] ?>）
            ...
            <button name="action" value="toggle_renew"
                ...
                <?= ($f['auto_renew'] ?? false) ? '🟢 自动续期 ON' : '🔴 自动续期 OFF' ?>
            </button>
        </form>
    <?php endforeach; ?>
</div>
