...
/* ==============================
   🔒 JSON 文件路径（自动判断）
   ============================== */
$write_dir = "/opt/render/project/.data/";   // Render 最安全持久目录
if (!is_dir($write_dir)) {
    $write_dir = __DIR__ . "/";             // 本地开发模式
}
...
<div class="alert success">✔ 添加成功！JSON 已保存</div>
<?php endif; ?>
...
<?php foreach ($foods as $f): 
    ...
    <div class="card <?= $c['status'] ?>" data-category="<?= $f['category'] ?>">
        ...
        <div class="name"><?= $f["name"] ?> 
            <?php if ($f["name_en"]): ?><span class="en"> / <?= $f["name_en"] ?></span><?php endif; ?>
        </div>
